<?php
// filepath: c:\MAMP\htdocs\wordpress\wp-content\themes\iltheme\inc\meta-boxes.php

function il_theme_add_custom_meta_boxes() {
    add_meta_box(
        'il_theme_coverimage_meta_box',
        __('Cover Image', 'iltheme'),
...
function il_theme_coverimage_meta_box_callback($post) {
    ...
    echo '<input type="button" id="il_theme_coverimage_button" class="button" value="' . __('Select Cover Image', 'iltheme') . '">';
...
function il_theme_highlight_meta_box_callback($post) {
    ...
    echo __('Highlight this post', 'iltheme');
    echo '</label>';
}
...
function il_theme_add_category_image_field($term) {
    ...
    <tr class="form-field">
        <th scope="row" valign="top">
            <label for="category_image"><?php _e('Category Image', 'iltheme'); ?></label>
        </th>
        <td>
            ...
            <input type="button" class="button" id="category_image_button" value="<?php _e('Upload/Add Image', 'iltheme'); ?>" />
            <input type="button" class="button" id="category_image_remove_button" value="<?php _e('Remove Image', 'iltheme'); ?>" />
            <p class="description"><?php _e('Upload or add an image for this category.', 'iltheme'); ?></p>
        </td>
    </tr>
    <script>
        jQuery(document).ready(function($) {
            $('#category_image_button').click(function(e) {
                e.preventDefault();
                var image = wp.media({
                    title: '<?php _e('Upload/Add Image', 'iltheme'); ?>',
                    multiple: false
                }).open()
                .on('select', function() {
